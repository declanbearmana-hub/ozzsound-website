...
(()=>{
  ...
  const renderProduct=p=>{const a=document.createElement('article');a.className='hireProduct';a.dataset.category=p.category||'other';a.dataset.inventoryId=p.id||'';const video=p.demo_video_url||p.video||'';const videoLabel=p.demo_video_label||'Watch this gear in action';const meta=[p.brand,p.model].filter(Boolean).join(' · ');a.innerHTML=`<div class="hireProductVisual"><div class="gearFallback">${icons[p.category]||'♫'}</div><small>${esc(p.category)}</small></div><div class="hireProductBody"><h3>${esc(p.name)}</h3>${meta?`<div class="hireProductMeta">${esc(meta)}</div>`:''}<p>${esc(p.description)}</p>${p.specs?`<details class="hireSpecs"><summary>Specifications & item details</summary><p>${esc(p.specs)}</p></details>`:''}${video?`<a class="hireDemo" href="${esc(video)}" target="_blank" rel="noopener">${esc(videoLabel)}</a>`:''}<div class="hireControls"><label><span>Qty</span><input class="hireQty" type="number" min="1" max="${Math.max(1,Number(p.available_quantity??p.quantity_owned??20))}" value="1"></label></div><button class="addHire" type="button">Add to hire list +</button></div>`;
