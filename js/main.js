...
(()=>{
  ...
  const equipmentExtraNames=['Karaoke','Bubble machine','Smoke machine','Laser lighting'];
  ...
  try{
    ...
    if(z&&new URLSearchParams(location.search).get('from')==='seasonal'){
      ...
      if(msg&&!msg.value){const lines=['Seasonal Event Builder','Season: '+(z.season||'Not chosen'),'Feel: '+(z.vibe||'Not chosen')];if(z.crowd)lines.push('Crowd: '+z.crowd);if((z.music||[]).length)lines.push('Music: '+z.music.join(', '));if(z.musicControl)lines.push('Music approach: '+z.musicControl);const seasonalOtherExtras=(z.extras||[]).filter(x=>!equipmentExtraNames.includes(x));if(seasonalOtherExtras.length)lines.push('Seasonal extras: '+seasonalOtherExtras.join(', '));if((z.moments||[]).length)lines.push('Important bits: '+z.moments.join(', '));if(z.strobe)lines.push('Strobe: '+z.strobe+(z.strobe==='Yes'?' · Organiser approval: '+(z.strobeApproval||'Not confirmed'):''));if(z.notes)lines.push('Anything else: '+z.notes);msg.value=lines.join('\n')}
    }
  }catch(e){}
  try{
  ...
  try{
    ...
    if(s&&['schools','School Function'].includes(new URLSearchParams(location.search).get('event'))){
      ...
      if(msg&&!msg.value){
        ...
        if(Array.isArray(s.extras)&&s.extras.length){const interactive=s.extras.filter(x=>!equipmentExtraNames.includes(x));if(interactive.length)lines.push('Interactive extras: '+interactive.join(', '));}
...
(()=>{
  ...
  const icons={sound:'🔊',lighting:'✦',effects:'✨',microphones:'🎤',karaoke:'🎙',dj:'◉'};
  const esc=s=>String(s||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const renderProduct=p=>{const a=document.createElement('article');a.className='hireProduct';a.dataset.category=p.category||'other';a.dataset.inventoryId=p.id||'';const video=p.demo_video_url||p.video||'';const videoLabel=p.demo_video_label||'Watch this gear in action';const similarVideo=/similar/i.test(videoLabel);const stock=Number(p.available_quantity??p.quantity_owned??0);const meta=[p.brand,p.model].filter(Boolean).join(' · ');const photos=(Array.isArray(p.photos)?p.photos:[]).filter(Boolean);let photoIndex=Math.max(0,photos.indexOf(p.image));if(photoIndex<0)photoIndex=0;a.innerHTML=`<div class="hireProductVisual"><div class="gearFallback">${icons[p.category]||'♫'}</div><small>${esc(p.category)}</small>${photos.length>1?'<button class="hirePhotoPrev" type="button" aria-label="Previous equipment photo">‹</button><button class="hirePhotoNext" type="button" aria-label="Next equipment photo">›</button><span class="hirePhotoCount">1 / '+photos.length+'</span>':''}</div><div class="hireProductBody"><h3>${esc(p.name)}</h3>${meta?`<div class="hireProductMeta">${esc(meta)}</div>`:''}<p>${esc(p.description)}</p>${p.id==='b5a62409-c530-40c8-bd66-6cde5403a773'?'<div class="hireItemNotice"><strong>SUBWOOFER ONLY</strong><span>The RCF top speaker shown above the subwoofer in some photos is not included. Select the RCF speaker separately if required.</span></div>':''}<div class="hireStock" data-stock-count>${stock} in stock</div>${p.specs?`<details class="hireSpecs"><summary>Specifications & item details</summary><p>${esc(p.specs)}</p></details>`:''}${video?`<a class="hireDemo" href="${esc(video)}" target="_blank" rel="noopener">▶ ${esc(videoLabel)}</a>${similarVideo?`<div class="hireVideoDisclaimer">Example video: the product shown is extremely similar to the equipment you would be hiring. Appearance or minor features may vary.</div>`:''}`:''}<div class="hireControls"><label><span>Qty</span><input class="hireQty" type="number" min="1" max="${Math.max(1,Number(p.available_quantity??p.quantity_owned??20))}" value="1"></label></div><button class="addHire" type="button">Add to hire list +</button></div>`;
    if(p.image){const im=new Image();const v=a.querySelector('.hireProductVisual');const showPhoto=i=>{if(!photos.length)return;photoIndex=(i+photos.length)%photos.length;a.classList.toggle('hireSecondaryPhoto',photoIndex>0);im.src=photos[photoIndex];const count=v.querySelector('.hirePhotoCount');if(count)count.textContent=(photoIndex+1)+' / '+photos.length};im.onload=()=>{v.querySelector('.gearFallback')?.remove();im.alt=p.name||'Ozzsound hire gear';if(!im.parentNode)v.prepend(im)};a.classList.remove('hireSecondaryPhoto');im.src=p.image;v.querySelector('.hirePhotoPrev')?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();showPhoto(photoIndex-1)});v.querySelector('.hirePhotoNext')?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();showPhoto(photoIndex+1)});if(photos.length>1){let touchX=null;v.addEventListener('touchstart',e=>{if(e.touches.length===1)touchX=e.touches[0].clientX},{passive:true});v.addEventListener('touchend',e=>{if(touchX===null)return;const dx=e.changedTouches[0].clientX-touchX;touchX=null;if(Math.abs(dx)<42)return;showPhoto(photoIndex+(dx<0?1:-1))},{passive:true})}}
  ...
  async function loadLiveInventory(){
    ...
    try{
      const url=cfg.supabaseUrl+'/rest/v1/equipment_inventory?select=id,name,category,brand,model,customer_description,description,quantity_owned,primary_photo_path,photo_paths,demo_video_url,demo_video_label&active=eq.true&show_on_gear_hire=eq.true&archived_at=is.null&order=sort_order.asc,name.asc';
      ...
      products=rows.map(x=>{const rawPhotos=[x.primary_photo_path,...(Array.isArray(x.photo_paths)?x.photo_paths:[])].filter(Boolean);const uniquePhotos=[...new Set(rawPhotos)];const photos=uniquePhotos.map(path=>cfg.supabaseUrl+'/storage/v1/object/public/inventory-media/'+encodeURIComponent(path).replace(/%2F/g,'/'));return {id:x.id,name:x.name,brand:x.brand||'',model:x.model||'',category:String(x.category||'other').toLowerCase(),description:x.customer_description||x.description||'',specs:x.description||'',quantity_owned:x.quantity_owned,demo_video_url:x.demo_video_url||'',demo_video_label:x.demo_video_label||'',photos,image:photos[0]||''}});const allGearOrder={effects:0,lighting:1,sound:2};products.sort((a,b)=>(allGearOrder[a.category]??1)-(allGearOrder[b.category]??1)||a.name.localeCompare(b.name));
      catalogue.innerHTML='';
      products.forEach(p=>catalogue.appendChild(renderProduct(p)));
      syncGearFilters();
