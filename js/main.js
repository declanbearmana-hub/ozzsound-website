...
(()=>{
  ...
  const renderProduct=p=>{const a=document.createElement('article');a.className='hireProduct';a.dataset.category=p.category||'other';a.dataset.inventoryId=p.id||'';const video=p.demo_video_url||p.video||'';const videoLabel=p.demo_video_label||'Watch this gear in action';const similarVideo=/similar/i.test(videoLabel);const stock=Number(p.available_quantity??p.quantity_owned??0);const meta=[p.brand,p.model].filter(Boolean).join(' · ');const photos=(Array.isArray(p.photos)?p.photos:[]).filter(Boolean);let photoIndex=Math.max(0,photos.indexOf(p.image));if(photoIndex<0)photoIndex=0;a.innerHTML=`<div class="hireProductVisual"><div class="gearFallback">${icons[p.category]||'♫'}</div><small>${esc(p.category)}</small>${photos.length>1?'<button class="hirePhotoPrev" type="button" aria-label="Previous equipment photo">‹</button><button class="hirePhotoNext" type="button" aria-label="Next equipment photo">›</button><span class="hirePhotoCount">1 / '+photos.length+'</span>':''}</div><div class="hireProductBody"><h3>${esc(p.name)}</h3>${meta?`<div class="hireProductMeta">${esc(meta)}</div>`:''}<p>${esc(p.description)}</p><div class="hireStock" data-stock-count>${stock} in stock</div>${p.specs?`<details class="hireSpecs"><summary>Specifications & item details</summary><p>${esc(p.specs)}</p></details>`:''}${video?`<a class="hireDemo" href="${esc(video)}" target="_blank" rel="noopener">▶ ${esc(videoLabel)}</a>${similarVideo?`<div class="hireVideoDisclaimer">Example video: the product shown is extremely similar to the equipment you would be hiring. Appearance or minor features may vary.</div>`:''}`:''}<div class="hireControls"><label><span>Qty</span><input class="hireQty" type="number" min="1" max="${Math.max(1,Number(p.available_quantity??p.quantity_owned??20))}" value="1"></label></div><button class="addHire" type="button">Add to hire list +</button></div>`;
    if(p.image){const im=new Image();const v=a.querySelector('.hireProductVisual');const showPhoto=i=>{if(!photos.length)return;photoIndex=(i+photos.length)%photos.length;im.src=photos[photoIndex];const count=v.querySelector('.hirePhotoCount');if(count)count.textContent=(photoIndex+1)+' / '+photos.length};im.onload=()=>{v.querySelector('.gearFallback')?.remove();im.alt=p.name||'Ozzsound hire gear';if(!im.parentNode)v.prepend(im)};im.src=p.image;v.querySelector('.hirePhotoPrev')?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();showPhoto(photoIndex-1)});v.querySelector('.hirePhotoNext')?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();showPhoto(photoIndex+1)})}
