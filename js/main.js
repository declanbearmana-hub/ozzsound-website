...
(()=>{
  ...
  const strobeSafety=document.querySelector('#strobeSafety'), strobeApproval=document.querySelector('#strobeApproval');
  ...
  function sync(){cartItems.innerHTML='';const vals=[...cart.values()];empty.hidden=vals.length>0;vals.forEach(x=>{const r=document.createElement('div');r.className='hireCartRow';r.innerHTML=`<strong>${esc(x.name)}</strong><span>Qty ${x.qty}</span><button type="button" aria-label="Remove ${esc(x.name)}">×</button>`;r.querySelector('button').onclick=()=>{cart.delete(x.id);sync()};cartItems.appendChild(r)});const hasStrobe=vals.some(x=>x.id==='strobe');if(strobeSafety)strobeSafety.hidden=!hasStrobe;if(!hasStrobe&&strobeApproval)strobeApproval.value='';const total=vals.reduce((n,x)=>n+x.qty,0);summary.textContent=vals.length?`${total} item${total===1?'':'s'} · ${vals.length} gear type${vals.length===1?'':'s'} selected`:'Nothing selected yet';enquire.classList.toggle('ready',vals.length>0)}
  ...
  enquire?.addEventListener('click',e=>{const vals=[...cart.values()];if(!vals.length){e.preventDefault();summary.textContent='Choose at least one item before continuing';return;}if(!hireDate?.value){e.preventDefault();summary.textContent='Choose a hire date before continuing';document.querySelector('#gearCalendar')?.scrollIntoView({behavior:'smooth',block:'center'});return;}const hasStrobe=vals.some(x=>x.id==='strobe');if(hasStrobe&&!strobeApproval?.value){e.preventDefault();summary.textContent='Please confirm the strobe-lighting safety question';strobeSafety?.scrollIntoView({behavior:'smooth',block:'center'});return;}const payload={items:vals,overallPeriod:period?.value||'Not specified',hireDate:hireDate.value,strobeApproval:hasStrobe?(strobeApproval?.value||'Not confirmed'):'Not applicable'};try{localStorage.setItem('ozzsoundGearHire',JSON.stringify(payload));}catch(err){}if(eventType)eventType.value='Gear Hire';if(eventDate)eventDate.value=hireDate.value;if(message){const lines=['Gear hire request:',...vals.map(x=>`- ${x.name} — Qty ${x.qty}`),`Package hire period: ${payload.overallPeriod}`,`Hire date: ${payload.hireDate}`];if(hasStrobe)lines.push(`Strobe organiser approval: ${payload.strobeApproval}`);message.value=lines.join('\n');}});
  sync();
})();
