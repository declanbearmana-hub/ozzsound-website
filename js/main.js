...
(()=>{
  ...
  const renderProduct=p=>{const a=document.createElement('article');a.className='hireProduct';a.dataset.category=p.category||'other';a.dataset.inventoryId=p.id||'';const video=p.demo_video_url||p.video||'';const videoLabel=p.demo_video_label||'Watch this gear in action';const similarVideo=/similar/i.test(videoLabel);const stock=Number(p.available_quantity??p.quantity_owned??0);const meta=[p.brand,p.model].filter(Boolean).join(' · ');a.innerHTML=`<div class="hireProductVisual"><div class="gearFallback">${icons[p.category]||'♫'}</div><small>${esc(p.category)}</small></div><div class="hireProductBody"><h3>${esc(p.name)}</h3>${meta?`<div class="hireProductMeta">${esc(meta)}</div>`:''}<p>${esc(p.description)}</p><div class="hireStock" data-stock-count>${stock} in stock</div>${p.specs?`<details class="hireSpecs"><summary>Specifications & item details</summary><p>${esc(p.specs)}</p></details>`:''}${video?`<a class="hireDemo" href="${esc(video)}" target="_blank" rel="noopener">▶ ${esc(videoLabel)}</a>${similarVideo?`<div class="hireVideoDisclaimer">Example video: the product shown is extremely similar to the equipment you would be hiring. Appearance or minor features may vary.</div>`:''}`:''}<div class="hireControls"><label><span>Qty</span><input class="hireQty" type="number" min="1" max="${Math.max(1,Number(p.available_quantity??p.quantity_owned??20))}" value="1"></label></div><button class="addHire" type="button">Add to hire list +</button></div>`;
  ...
  async function loadLiveInventory(){
    ...
    try{
      ...
      products=rows.map(x=>({id:x.id,name:x.name,brand:x.brand||'',model:x.model||'',category:String(x.category||'other').toLowerCase(),description:x.customer_description||x.description||'',specs:x.description||'',quantity_owned:x.quantity_owned,demo_video_url:x.demo_video_url||'',demo_video_label:x.demo_video_label||'',image:(x.primary_photo_path||(x.photo_paths||[])[0])?cfg.supabaseUrl+'/storage/v1/object/public/inventory-media/'+encodeURIComponent(x.primary_photo_path||(x.photo_paths||[])[0]).replace(/%2F/g,'/'):''}));const allGearOrder={effects:0,lighting:1,sound:2};products.sort((a,b)=>(allGearOrder[a.category]??1)-(allGearOrder[b.category]??1)||a.name.localeCompare(b.name));
      catalogue.innerHTML='';
      products.forEach(p=>catalogue.appendChild(renderProduct(p)));
      syncGearFilters();
