...
(()=>{
  ...
  const demoVideos={
    'CHAUVET DJ Intimidator Scan 110':'https://www.youtube.com/watch?v=FtMcYuwlZIY',
    'RCF SUB 705-AS II Active Subwoofer + Cover':'https://www.youtube.com/watch?v=TpMen0nM2hI'
  };
  const esc=s=>String(s||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const renderProduct=p=>{const a=document.createElement('article');a.className='hireProduct';a.dataset.category=p.category||'other';a.dataset.inventoryId=p.id||'';const video=p.video||demoVideos[p.name]||'';const meta=[p.brand,p.model].filter(Boolean).join(' · ');a.innerHTML=`<div class="hireProductVisual"><div class="gearFallback">${icons[p.category]||'♫'}</div><small>${esc(p.category)}</small></div><div class="hireProductBody"><h3>${esc(p.name)}</h3>${meta?`<div class="hireProductMeta">${esc(meta)}</div>`:''}<p>${esc(p.description)}</p>${p.specs?`<details class="hireSpecs"><summary>Specifications & item details</summary><p>${esc(p.specs)}</p></details>`:''}${video?`<a class="hireDemo" href="${esc(video)}" target="_blank" rel="noopener">▶ Watch this gear in action</a>`:''}<div class="hireControls"><label><span>Qty</span><input class="hireQty" type="number" min="1" max="${Math.max(1,Number(p.available_quantity??p.quantity_owned??20))}" value="1"></label></div><button class="addHire" type="button">Add to hire list +</button></div>`;
  ...
  async function loadLiveInventory(){
    ...
    try{
      const url=cfg.supabaseUrl+'/rest/v1/equipment_inventory?select=id,name,category,brand,model,customer_description,description,quantity_owned,primary_photo_path,photo_paths&active=eq.true&show_on_gear_hire=eq.true&archived_at=is.null&order=sort_order.asc,name.asc';
