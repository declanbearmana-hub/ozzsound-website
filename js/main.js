...
(()=>{
  ...
  async function loadLiveInventory(){
    ...
    try{
      ...
      products=rows.map(x=>({id:x.id,name:x.name,brand:x.brand||'',model:x.model||'',category:String(x.category||'other').toLowerCase(),description:x.customer_description||x.description||'',specs:x.description||'',quantity_owned:x.quantity_owned,demo_video_url:x.demo_video_url||'',demo_video_label:x.demo_video_label||'',image:(x.primary_photo_path||(x.photo_paths||[])[0])?cfg.supabaseUrl+'/storage/v1/object/public/inventory-media/'+encodeURIComponent(x.primary_photo_path||(x.photo_paths||[])[0]).replace(/%2F/g,'/'):''}));
      catalogue.innerHTML='';
      products.forEach(p=>catalogue.appendChild(renderProduct(p)));
      syncGearFilters();
