...
function smartLogic(){
 ...
 // Only ask for extra reception detail when a selected moment actually needs it.
 const receptionDetails=document.querySelector('#smartReceptionDetails');
 if(receptionDetails){
   const special=form.querySelector('#specialDances')?.checked;
   const other=form.querySelector('#otherMoment')?.checked;
   const existingSpecial=form.elements.specialDanceDetails?.value||'';
   const existingOther=form.elements.otherMomentDetails?.value||'';
   let html='';
   if(special)html+=`<label class="wedField"><span>Which special dance? <small style="font-weight:400;color:#7f8b9d">(optional)</small></span><input name="specialDanceDetails" value="${existingSpecial.replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;')}" placeholder="e.g. Father/daughter — song can be decided later"></label>`;
   if(other)html+=`<label class="wedField"><span>What's the special moment? <small style="font-weight:400;color:#7f8b9d">(optional)</small></span><input name="otherMomentDetails" value="${existingOther.replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;')}" placeholder="Tell us briefly — or leave it for later"></label>`;
   receptionDetails.innerHTML=html;
 }
